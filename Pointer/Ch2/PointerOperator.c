...
void PointerExample()
{
	...
	// 주소(포인터) 자체를 더하기 vs 주소를 참조해서 더하기
	// 변수를 사용하는데, 컴퓨터가 어딘가에 기억하고 있다. 주소라는 이름으로 기억하고 있다.
	// 변수의 이름으로 값을 가져올 수 있고 주소로부터 값을 가져올 수도 있다.

	// 주소 형태로 가져오는 방식(참조 형식)		- 프로그램이 종료되거나, 직접 메모리를 해제하기 전까지 게속 남습니다. - 힙(Heap) 영역
	// 변수의 이름으로 가져오는 방식(값 형식)		- {} 코드 영역이 끝나는 시점에 메모리가 해제됩니다.				  - 스택(Stack) 영역

	// 모든 코드를 항상 저장하면 안 될까요?
	// 게임 점수를 일정 값만큼 증가시키는 코드를 작성하고 싶어요.
	// 저장되어야 할 데이터 : 게임 점수,
	// 한 번만 사용하고 지우면 좋을 것 같다 : 게임 점수를 증가하고 싶은 가중치
	// 10분 실행된다.

	// 모든 코드를 한 번만 사용하고 저장을 안 하면?
	...
	// 주소를 더하면, 주소의 값이 가지고 있는 크기만큼 증가한다.
	// 배열과 포인터의 관계 중요(★)
}